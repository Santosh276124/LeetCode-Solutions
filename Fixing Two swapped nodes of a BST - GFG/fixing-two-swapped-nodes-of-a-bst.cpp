...
class Solution {
  ...
  void inorderToSwap(struct Node* root , int &ind, vector<int> &in)
  {
      if(root == NULL || ind >= in.size()) return;
      
      inorderToSwap(root->left, ind, in);
      ...
      inorderToSwap(root->right, ind, in);
